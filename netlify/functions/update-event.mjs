...
import { updateCalendarEvent } from "../../server/calendarService.js";
...
export default async (req, context) => {
  ...
  try {
    ...
    const { calendarKey, eventId } = context.params;
    const event = await updateCalendarEvent(calendarKey, eventId, input, { env: getFunctionEnv() });
    return Response.json({ event });
  } catch (error) {
    return Response.json({ error: error.message || "Unable to update event." }, { status: 400 });
  }
};

export const config = {
  path: "/api/events/:calendarKey/:eventId",
  method: ["PATCH"]
};
